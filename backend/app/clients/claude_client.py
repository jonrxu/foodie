    ...
    def analyze_meal_text(self, summary: str) -> MealAnalysisResult:
        ...
        try:
            message = self._client.messages.create(
                ...
                max_tokens=512,
                system=_MEAL_ANALYSIS_SYSTEM,
                messages=[{"role": "user", "content": f"Meal: {summary}"}],
            )
            return self._parse_analysis(message.content[0].text, fallback_summary=summary)
            ...
            message = self._client.messages.create(
                ...
                messages=[{
                    ...
                    "content": [
                        ...
                        {"type": "text", "text": "What meal is in this image?"},
                    ],
                }],
            )
            return self._parse_analysis(message.content[0].text, fallback_summary="Photo meal")
        except Exception as exc:
            logger.warning("Claude image analysis failed: %s", exc)
            return self._fallback_analysis("Photo meal")
    ...
    def generate_weekly_cart(
        ...
        try:
            ...
            message = self._client.messages.create(
                model="claude-sonnet-4-6",
                max_tokens=1024,
                system=_WEEKLY_CART_SYSTEM,
                messages=[{"role": "user", "content": prompt}],
            )
            return self._parse_weekly_cart(message.content[0].text)
        except Exception as exc:
            logger.warning("Claude weekly cart failed: %s", exc)
            return WeeklyCartResult(title="Weekly grocery list", items=[])
