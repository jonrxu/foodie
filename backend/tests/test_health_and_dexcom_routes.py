...
def test_start_status_callback_success_sync_disconnect_flow(client: TestClient, auth_headers) -> None:
    user = "user-flow"
    ...
    status_pending = client.get("/dexcom/connect/status", headers=headers)
    ...
    status_connected = client.get("/dexcom/connect/status", headers=headers)
    ...
    status_after_sync = client.get("/dexcom/connect/status", headers=headers)
    assert status_after_sync.status_code == 200
    assert status_after_sync.json()["last_sync_at"] is not None

    disconnect = client.post("/dexcom/disconnect", headers=headers)
    assert disconnect.status_code == 200
    assert disconnect.json()["status"] == "disconnected"

    status_after_disconnect = client.get("/dexcom/connect/status", headers=headers)
    assert status_after_disconnect.status_code == 200
    assert status_after_disconnect.json()["status"] == "disconnected"


def test_callback_error_marks_connection_error_and_redirects(client: TestClient, auth_headers) -> None:
    user = "user-error"
    headers = auth_headers(user)
    start = client.post("/dexcom/connect/start", headers=headers)
    ...
    status = client.get("/dexcom/connect/status", headers=headers)
...
def test_sync_requires_connected_state(client: TestClient, auth_headers) -> None:
    response = client.post("/dexcom/sync", headers=auth_headers("user-not-connected"))
    assert response.status_code == 409
    assert response.json()["error"]["code"] == "dexcom_not_connected"


def test_users_are_isolated(client: TestClient, auth_headers) -> None:
    client.post("/dexcom/connect/start", headers=auth_headers("user-a"))

    status_b = client.get("/dexcom/connect/status", headers=auth_headers("user-b"))
    assert status_b.status_code == 200
    assert status_b.json()["status"] == "disconnected"


def test_connection_persists_across_service_cache_reset(client: TestClient, auth_headers) -> None:
    user = "persisted-user"
    headers = auth_headers(user)
    start = client.post("/dexcom/connect/start", headers=headers)
    state = _extract_state(start.json()["authorization_url"])
    ...
    persisted = client.get("/dexcom/connect/status", headers=headers)
    assert persisted.status_code == 200
    assert persisted.json()["status"] == "connected"


def test_weekly_summary_and_readings_routes_return_persisted_glucose(client: TestClient, auth_headers) -> None:
    ...
    summary = client.get("/cgm/summary/weekly", headers=headers)
    ...
    readings = client.get("/cgm/readings", headers=headers)
